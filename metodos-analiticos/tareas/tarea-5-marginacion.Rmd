...
```{r}
(as.matrix(marg_tabla_esc)[1,] %*% ((dvs_marg$v))[,1])/(sigma_1)
dvs_marg$u[1,1]
```
...
xx <- (as.matrix(marg_tabla_esc)[,] %*% ((dvs_marg$v))[,1])/(sigma_1)
xx2 <- (xx-mean(xx))/sd(xx)
var(xx2)
mean(xx2)

mean(xx2 - marginacion$IM)
```
